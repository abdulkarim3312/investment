                        ...
                        @foreach ($blogs as $item)
                            <tr>
                                ...
                                <td class="align-middle text-center">
                                    <span>
                                    <img src="{{ asset('upload/blog_image/'.$item->image) }}" alt="" height="40">
                                    </span>
                                </td>
                                <td class="align-middle text-center">
                                @if ($item->status == 1)
                                ...
                                <td>
                                    <a class="btn waves-effect icon_size waves-light btn btn-primary btn-sm btn-sm-custom ml-1 mb-0" title="View Blog Details" href="{{ route('blog.show', $item->id) }}" class="btn btn-info"><i class="fa fa-eye"></i></a>
                                    <a class="btn waves-effect icon_size waves-light btn-success btn-sm btn-sm-custom ml-1  mb-0" title="Edit Blog Details" href="{{ route('blog.edit', $item->id) }}">
                                        <i class="fa fa-edit"></i></i>
                                    </a>
                                    <button class="btn waves-effect icon_size waves-light btn-danger btn-sm btn-sm-custom ml-1 mb-0 text-white deleteItem" title="Delete Blog" data-id="{{$item->id}}">
                                        <i class="fa fa-trash"></i>
                                    </button>
                                    <form id="deleteForm{{$item->id}}" action="{{route('blog.destroy', [$item->id])}}" method="post" style="display:none">
                                        @csrf
                                        @method('delete')
