        ...
        <div class="card-header pb-0">
            <div class="row">
                <div class="col-lg-6 col-7">
                    <h6>Agreement Procedure List</h6>
                </div>
                <div class="col-lg-6 col-5 my-auto text-end">
                    <div class="float-lg-end">
                        <a href="{{ route('agreement-procedure.create') }}" class="btn btn-primary float-right">Add new</a>
                    </div>
                </div>
            </div>
        </div>
                        ...
                        @foreach ($type as $item)
                            <tr>
                                ...
                                <td class="align-middle text-center text-sm">
                                    <p class="text-center text-xs font-weight-medium mb-0">{{ $item->category->name ?? ''}}</p>
                                </td>
                                ...
                                <td>
                                    <a class="btn waves-effect icon_size waves-light btn btn-primary btn-sm btn-sm-custom ml-1 mb-0" title="View Blog Details" href="{{ route('agreement-procedure.show', $item->id) }}" class="btn btn-info"><i class="fa fa-eye"></i></a>
                                    <a class="btn waves-effect icon_size waves-light btn-success btn-sm btn-sm-custom ml-1  mb-0" title="Edit Service Details" href="{{ route('agreement-procedure.edit', $item->id) }}">
                                    ...
                                    <form id="deleteForm{{$item->id}}" action="{{route('agreement-procedure.destroy', [$item->id])}}" method="post" style="display:none">
                                        @csrf
                                        @method('delete')
