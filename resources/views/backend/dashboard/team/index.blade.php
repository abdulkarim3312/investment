            ...
            <div class="row">
                ...
                <div class="col-lg-6 col-5 my-auto text-end">
                    <div class="float-lg-end">
                        <a href="{{ route('team.create') }}" class="btn btn-primary float-right">Add new</a>
                    </div>
                </div>
            </div>
        </div>
                ...
                <table class="table table-flush dataTable-table" id="users_table">
                    <thead>
                        <tr>
                            ...
                            <th width="10%" class="text-center text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 ps-2">Designation</th>
                    ...
                    <tbody>
                        @foreach ($teams as $item)
                            <tr>
                                ...
                                <td class="align-middle text-center text-sm">
                                    <p class="text-center text-xs font-weight-medium mb-0">{{ $item->designation ?? '' }}</p>
                                </td>
                                <td class="align-middle text-center">
                                    <span>
                                    <img src="{{ asset('upload/team_image/'.$item->image) }}" alt="" height="40">
                                    </span>
                                </td>
                                <td class="align-middle text-center">
                                @if ($item->status == 1)
                                ...
                                <td>
                                    <a class="btn waves-effect icon_size waves-light btn-success btn-sm btn-sm-custom ml-1  mb-0" title="Edit Service Details" href="{{ route('team.edit', $item->id) }}">
                                    ...
                                    <form id="deleteForm{{$item->id}}" action="{{route('team.destroy', [$item->id])}}" method="post" style="display:none">
                                        @csrf
                                        @method('delete')
